...
#define TMAG5273_DEFINE(inst, compat, _part)                                                       \
	...
	BUILD_ASSERT(!DT_NODE_HAS_PROP(DT_INST(inst, compat), i2c_startup_address) ||              \
			     ((uint8_t)DT_PROP_OR(DT_INST(inst, compat), i2c_startup_address, 0) < \
			      0x7F),                                                               \
	...
	static const struct tmag5273_config compat##_driver_cfg##inst = {                          \
		...
		.supply_gpio = GPIO_DT_SPEC_GET_OR(DT_INST(inst, compat), supply_gpios, {0}),      \
		.i2c_update = DT_NODE_HAS_PROP(DT_INST(inst, compat), i2c_startup_address),        \
		.i2c_startup_address = DT_PROP_OR(DT_INST(inst, compat), i2c_startup_address, 0),  \
